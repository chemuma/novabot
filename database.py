...
def _migrate(cur):
    """افزودن ستون‌های جدید به جدول‌های موجود (برای دیتابیس‌های قبلاً ساخته‌شده)."""
    cur.execute("PRAGMA table_info(satisfaction)")
    cols = [r["name"] for r in cur.fetchall()]
    if "group_message_id" not in cols:
        cur.execute("ALTER TABLE satisfaction ADD COLUMN group_message_id INTEGER")
    ...
    with db_cursor() as cur:
        cur.execute(
            """SELECT cart_items.id as cart_id, products.id as product_id, products.name,
                      products.price, products.category_id, cart_items.quantity
               FROM cart_items
...
def get_cart_item_count(user_id):
    """مجموع تعداد آیتم‌های سبد (برای مصرف ظرفیت کد تخفیف)"""
...
def set_order_ready_estimate(order_id, minutes):
    with db_cursor(commit=True) as cur:
        cur.execute("UPDATE orders SET ready_estimate = ? WHERE id = ?", (minutes, order_id))
...
def set_satisfaction_group_message(satisfaction_id, message_id):
    with db_cursor(commit=True) as cur:
        cur.execute(
            "UPDATE satisfaction SET group_message_id = ? WHERE id = ?", (message_id, satisfaction_id)
        )


def get_satisfaction_by_group_message(message_id):
    with db_cursor() as cur:
        cur.execute("SELECT * FROM satisfaction WHERE group_message_id = ?", (message_id,))
        return cur.fetchone()


def get_recent_satisfaction(limit=10):
    with db_cursor() as cur:
        cur.execute(
            """SELECT satisfaction.*, orders.display_number, users.name as user_name
               FROM satisfaction
               JOIN orders ON orders.id = satisfaction.order_id
               JOIN users ON users.id = satisfaction.user_id
               WHERE satisfaction.rating IS NOT NULL
               ORDER BY satisfaction.id DESC LIMIT ?""",
            (limit,),
        )
        return cur.fetchall()
